...
import static phamtuam.filemanager.source.controllers.misc.IntentUtils.ACTION_OPEN_DOCUMENT_TREE;
import static phamtuam.filemanager.source.controllers.provider.ExternalStorageProvider.ROOT_ID_SECONDARY;
...
public class SAFManager {
    ...
    public static String getRootUri(Uri uri) {
        if (isTreeUri(uri)) {
            return phamtuam.filemanager.source.controllers.model.DocumentsContract.getTreeDocumentId(uri);
        }
        return phamtuam.filemanager.source.controllers.model.DocumentsContract.getDocumentId(uri);
    }

    private static Uri buildDocumentUriMaybeUsingTree(Uri uri, String docId) {
        return phamtuam.filemanager.source.controllers.model.DocumentsContract.buildDocumentUriMaybeUsingTree(uri, docId);
    }

    private static boolean isTreeUri(Uri uri) {
        return phamtuam.filemanager.source.controllers.model.DocumentsContract.isTreeUri(uri);
    }
